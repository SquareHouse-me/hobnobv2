...
@section('page_title', __('voyager::generic.viewing').' '.$dataType->getTranslatedAttribute('') . 'Business Owners')

@section('page_header')
    <div class="container-fluid">
        <h1 class="page-title">
            <i class="{{ $dataType->icon }}"></i> Business Owners
        </h1>
        @can('add', app($dataType->model_name))
            <a href="{{ route('voyager.'.$dataType->slug.'.business.create') }}" class="btn btn-success btn-add-new">
                            ...
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tr>
                                    ...
                                    @foreach($dataType->browseRows as $row)
                                        @if($row->display_name == "Role" || $row->display_name == "Roles")
                                        @else
                                            <th>
                                ...
                                @foreach($dataTypeContent as $data)
                                    <tr>
                                        ...
                                        <td class="no-sort no-click bread-actions">
                                            ...
                                            @endforeach
                                            <?php
                                            $checkChatRoomAssignForUser = \Illuminate\Support\Facades\DB::table('assign_chat_room')
                                                ->where('user_id', $data->id)->first();
                                            ?>
                                            @if(empty($checkChatRoomAssignForUser))
                                                <a href="{{ route('voyager.assign.chat.room', $data->id) }}"
                                                   class="btn btn-danger pull-right" style="margin-right: 5px">
                                                    <i class="voyager-info-circled"></i><span style="padding-left: 5px">Assign Chat Room</span>
                                                </a>
                                            @endif
                                        </td>
                                    </tr>
    ...
    <script>
        $(document).ready(function () {
            $('.edit').each(function () {
                ...
                var newUrl = '/business_owner'; // Create new url
                var concatUrl = oldUrl.concat(newUrl);
                $(this).attr("href", concatUrl); // Set herf value
            });
        });
    </script>
@stop
